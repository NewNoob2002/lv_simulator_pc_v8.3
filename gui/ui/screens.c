  ...
  lv_obj_clear_flag(main_screen, LV_OBJ_FLAG_SCROLLABLE);
  {
    ...
    }
    {
...

      {
				...
				lv_obj_set_size(cont_satellite, 80, 50);
				lv_obj_clear_flag(cont_satellite, LV_OBJ_FLAG_SCROLLABLE);
				lv_obj_align(cont_satellite,  LV_ALIGN_LEFT_MID, 70, 0);
        // satellite_num
        ...
        lv_obj_align(label_satellite_num, LV_ALIGN_LEFT_MID, -15, 0);
        ...
        lv_obj_align_to(label_satellite_num_real, label_satellite_num, LV_ALIGN_OUT_RIGHT_MID, 10, 10);
...
void tick_screen_main()
{
  lv_obj_clear_flag(objects.main, LV_OBJ_FLAG_HIDDEN);
	/*Battery*/
	lv_update_battery(&batteryState, &objects.page_main.status_bar_obj.battery);
  Page_Main_Update();
}
...
void create_screen_work_config()
{
...

  {
    ...
    // First child: Back button
    {
      ...
      lv_obj_add_event_cb(button_batteryStatus, action_switch_to_battery, LV_EVENT_PRESSED, (void *)0);
			
			lv_obj_t *label_batteryStatus = lv_label_create(button_batteryStatus);
      lv_label_set_text(label_batteryStatus, LV_SYMBOL_CHARGE);
      lv_obj_set_style_text_font(label_batteryStatus, &lv_font_montserrat_16, 0);
    ...
    // Third child: 4G status and switch
    {
      ...
      lv_obj_align(icon_static, LV_ALIGN_TOP_LEFT, 95, 20);

      // Static status label
      lv_obj_t *button_static = lv_btn_create(parent_obj);
      lv_obj_align_to(button_static, icon_static, LV_ALIGN_RIGHT_MID, 45, -5);
      ...
      lv_obj_align_to(icon_4g, icon_static, LV_ALIGN_BOTTOM_MID, 0, 65);

      lv_obj_t *button_4g = lv_btn_create(parent_obj);
      lv_obj_align_to(button_4g, icon_4g, LV_ALIGN_LEFT_MID, 30, -10);
  ...
  lv_obj_set_size(screen_batteryInfo, 294, 126);
	{
      ...
      lv_obj_add_event_cb(button_batteryInfo_back, action_switch_to_work_config, LV_EVENT_PRESSED, (void *)0);
      ...
      lv_obj_set_style_text_color(battery_charge_temp, lv_color_hex(0xffffff), 0);
		...
		{
			//charge chart
			lv_obj_t *chart = lv_chart_create(parent_obj);
			lv_obj_set_size(chart, 250, 70);
			lv_obj_align(chart, LV_ALIGN_BOTTOM_MID, 0, -10);
			
			lv_chart_set_type(chart, LV_CHART_TYPE_LINE);
			
			battery_chart.max_points = 100;
			battery_chart.time_base = 1.0f;
			lv_chart_set_point_count(chart, battery_chart.max_points);
			
			    // 设置X轴范围（时间）
			lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_X, 
													0, battery_chart.max_points - 1);
			// 设置Y轴范围
			// 电压范围：6.0V - 8.4V，电流范围：0A - 2A
			lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, 6000, 8400);  // 以毫伏为单位
			lv_chart_set_range(chart, LV_CHART_AXIS_SECONDARY_Y, 0, 2000); // 以毫安为单位
			
			    // 添加数据系列
			battery_chart.voltage_series = lv_chart_add_series(chart, 
                                                      lv_palette_main(LV_PALETTE_RED), 
                                                      LV_CHART_AXIS_PRIMARY_Y);
    
			battery_chart.current_series = lv_chart_add_series(chart, 
                                                      lv_palette_main(LV_PALETTE_BLUE), 
                                                      LV_CHART_AXIS_SECONDARY_Y);
																											
																											   // 初始化数据点
    battery_chart.point_count = 0;
    for(int i = 0; i < battery_chart.max_points; i++) {
        battery_chart.voltage_series->y_points[i] = i*100;
        battery_chart.current_series->y_points[i] = i;
    }
    lv_chart_refresh(chart);
    // 添加坐标轴标签
//    lv_obj_t *x_label = lv_label_create(lv_scr_act());
//    lv_label_set_text(x_label, "Time (s)");
//    lv_obj_align_to(x_label, chart, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
//    
//    lv_obj_t *y1_label = lv_label_create(lv_scr_act());
//    lv_label_set_text(y1_label, "Vol (mV)");
//    lv_obj_align_to(y1_label, chart, LV_ALIGN_OUT_LEFT_TOP, -30, 0);
//    
//    lv_obj_t *y2_label = lv_label_create(lv_scr_act());
//    lv_label_set_text(y2_label, "Cur (mA)");
//    lv_obj_align_to(y2_label, chart, LV_ALIGN_OUT_RIGHT_TOP, 30, 0);
...
{
	lv_obj_clear_flag(objects.batteryInfo, LV_OBJ_FLAG_HIDDEN);
	if(batteryState.chargeStatus != notCharge)
	{
	...
	lv_label_set_text_fmt(objects.page_batteryInfo.label_battery_percent, "%0.2f%%", batteryState.Percent_f);
	lv_label_set_text_fmt(objects.page_batteryInfo.label_battery_voltage, "%0.2fV", batteryState.Voltage_f);
	lv_label_set_text_fmt(objects.page_batteryInfo.label_battery_temp, "%0.1fC", batteryState.Temp_f);
}
...
{
	lv_obj_clear_flag(objects.power_off, LV_OBJ_FLAG_HIDDEN);
	lv_update_battery(&batteryState, &objects.page_poweroff.battery);
}
...
{
	lv_obj_clear_flag(objects.power_off_charge, LV_OBJ_FLAG_HIDDEN);
	/*Battery*/
	lv_update_battery_off(&batteryState, &objects.page_poweroff_charge.battery);
	lv_label_set_text_fmt(objects.page_poweroff_charge.label_charge_time, "%s", "0:00:00");
	
	/*ARC*/
	uint8_t target_percent = (Power_Monitor.PowerKey_PressCount * 2);

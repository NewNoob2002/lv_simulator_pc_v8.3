...
void action_turn_onoff_static(lv_event_t *e)
{
	...
	if (code == LV_EVENT_VALUE_CHANGED)
	{
		if (lv_obj_has_state(obj, LV_STATE_CHECKED))
		{
			...
			systemInfo.mcu_operation_flag = 1;
...
void action_turn_onoff_4g(lv_event_t *e)
{
	...
	if (code == LV_EVENT_VALUE_CHANGED)
	{
		if (lv_obj_has_state(obj, LV_STATE_CHECKED))
		{
			...
			systemInfo.radio_status = 0;
			systemInfo.gprs_status = 1;
			systemInfo.NtripClient_status = 0;
			systemInfo.NtripServer_status = 1;
		}
		else
		{
			if(!systemInfo.gprs_status)
				return;
			systemInfo.mcu_operation_flag = 1;
			systemInfo.gprs_status = 0;
			systemInfo.NtripClient_status = 0;
			systemInfo.NtripServer_status = 0;
		}
	}
}
...
void action_switch_to_battery(lv_event_t *e)
{
	loadScreen(SCREEN_ID_BATTERY_INFO);
}

lv_obj_t *mbox1 = NULL;

static void msgbox_event_cb(lv_timer_t *timer)
{
...
void action_radio_roller(lv_event_t *e)
{
	...
	if (code == LV_EVENT_VALUE_CHANGED)
	{
		...
		char display_str[32];
		snprintf(display_str, sizeof(display_str), "Set as %s(%d)", radio_ch_str, radio_ch_index);
		mbox1 = lv_msgbox_create(objects.radio_config, NULL, display_str, NULL, false);
...
{
	systemInfo.mcu_operation_flag = 1;
	systemInfo.radio_channel = radio_ch_index;
	systemInfo.radio_protocol = radio_proc_index;
	systemInfo.radio_mode = 0;
	systemInfo.radio_status = 1;
	systemInfo.gprs_status = 0;
	systemInfo.NtripClient_status = 0;
	systemInfo.NtripServer_status = 0;
	systemInfo.work_mode = 1;
...
void lv_update_battery(pBatteryState p_batteryState, battery_obj *battery)
{
...
void lv_update_battery_off(pBatteryState p_batteryState, battery_obj *battery)
{
	uint8_t battery_percentage = p_batteryState->Percent;
	/*battery*/
	lv_label_set_text_fmt(battery->label_text, "%d%%", battery_percentage);

	bool Is_BattCharging = p_batteryState->chargeStatus == 1 || p_batteryState->chargeStatus == 2;
	bool is_FullCharge = (battery_percentage >= 100);
	lv_obj_t *battery_useage = battery->label_useage;
...
static void page_mainTopUpdate(status_bar *bar)
{
	...
	else
	{
		...
		{
			lv_obj_set_style_text_color(bar->icon_wifi, lv_palette_main(LV_PALETTE_RED), 0);
			lv_obj_set_style_text_color(bar->icon_bluetooth, lv_palette_main(LV_PALETTE_RED), 0);
		}
		else
		{
	...
	{
		lv_obj_set_style_text_color(bar->icon_radio_status, lv_palette_main(LV_PALETTE_GREEN), 0);
	}
	else
	{
		lv_obj_set_style_text_color(bar->icon_radio_status, lv_palette_main(LV_PALETTE_RED), 0);
	}
	// SD
...
void Page_RadioConfig_Update()
{
	...
	if (systemInfo.radio_status)
	{
		lv_obj_set_style_text_color(objects.page_radio.lable_radio_status, lv_palette_main(LV_PALETTE_GREEN), 0);
	}
	else
	{
		lv_obj_set_style_text_color(objects.page_radio.lable_radio_status, lv_palette_main(LV_PALETTE_RED), 0);
	}
...
void Page_StaticRecord_Update()
{
	if (systemInfo.record_status)
	{
		if (!lv_obj_has_state(objects.page_static.switch_static_onoff, LV_STATE_CHECKED))
		{
			lv_obj_add_state(objects.page_static.switch_static_onoff, LV_STATE_CHECKED);
		}
		lv_obj_set_style_text_color(objects.page_static.icon_static_name, lv_palette_main(LV_PALETTE_GREEN), 0);
		lv_label_set_text_fmt(objects.page_static.label_static_name, "REC: %s", systemInfo.record_name);
		lv_label_set_text_fmt(objects.page_static.label_static_type, "Type: %s", systemInfo.record_type == 1 ? "XYZ" : "Rinex3.02");
		lv_label_set_text_fmt(objects.page_static.label_static_time, "Time: %d hour", systemInfo.record_interval);
		lv_obj_set_style_text_color(objects.page_static.icon_sd, lv_palette_main(LV_PALETTE_GREEN), 0);
		lv_label_set_text_fmt(objects.page_static.label_sd, "RSC: %0.1fMB", systemInfo.record_leftspace);
	}
	else
	{
		if (lv_obj_has_state(objects.page_static.switch_static_onoff, LV_STATE_CHECKED))
		{
			lv_obj_clear_state(objects.page_static.switch_static_onoff, LV_STATE_CHECKED);
		}

		lv_obj_set_style_text_color(objects.page_static.icon_static_name, lv_palette_main(LV_PALETTE_GREY), 0);
		...
		lv_obj_set_style_text_color(objects.page_static.icon_sd, lv_palette_main(LV_PALETTE_GREY), 0);
...
void Page_NtripServer_Update()
{
		...
		if (systemInfo.gprs_status)
		{
			lv_obj_set_style_text_color(objects.page_network.icon_4g, lv_palette_main(LV_PALETTE_LIGHT_BLUE), 0);
		...
		{
			lv_obj_set_style_text_color(objects.page_network.icon_4g, lv_palette_main(LV_PALETTE_GREY), 0);
		...
		if(systemInfo.NtripServer_status)
		{
			lv_label_set_text(objects.page_network.ip_ntrip_server, systemInfo.NtripServer_IP);
			lv_label_set_text(objects.page_network.name_ntrip_server, systemInfo.NtripServer_Mountpoint);
		}
		else
		{
...
void start_animation_to_value(lv_obj_t *arc, lv_anim_t *anim, int32_t target_value)
{
...
void arc_angle_anim(void *obj, int32_t v)
{
	...
	lv_label_set_text_fmt(objects.page_poweroff_charge.label_arc_percent, "%d%%", v);
	if (v >= 95)
	{
		lv_obj_set_style_arc_color((lv_obj_t *)obj, lv_palette_main(LV_PALETTE_GREEN), LV_PART_INDICATOR);
		if (v >= 100)
		{
	...
	{
		lv_obj_set_style_arc_color((lv_obj_t *)obj, lv_palette_main(LV_PALETTE_RED), LV_PART_INDICATOR);
